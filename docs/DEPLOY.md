...
Корень сайта (`https://www.gastrotruck.org/`) — экран выбора **Еда / Бар**. Меню бара: `/bar`, еда: `/food`.

Внутреннее приложение сотрудников (закупки): `/staff` — без отдельного поддомена.

Старый URL `/start` перенаправляет на главную.
