...
### Экран выбора «Еда / Бар» (QR на столах)

Пока на столах стоят QR с основным доменом, экран выбора вешают на **отдельный поддомен**:

1. **Vercel** → Project → **Settings** → **Domains** → добавьте, например, `menu.gastrotruck.org`.
2. У регистратора DNS: **CNAME** `menu` → `cname.vercel-dns.com` (точное значение — в Vercel).
3. После деплоя корень поддомена (`https://menu.gastrotruck.org/`) открывает экран выбора.
4. Проверка без поддомена: `https://ваш-домен/start`.

Когда будете готовы перенести QR на основной домен — напишите, переключим `www.gastrotruck.org/` на этот экран.

Переменная `MENU_CHOOSER_HOSTS` (через запятую) переопределяет список поддоменов. По умолчанию: `menu.gastrotruck.org`.
