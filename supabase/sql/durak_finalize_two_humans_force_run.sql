...
BEGIN
  ...
  SET status = 'playing'
  WHERE u.id = p_room_id
    AND u.status = 'waiting'
    AND (
      SELECT COUNT(*)::int
      FROM public.room_players rp
      WHERE rp.room_id = u.id AND NOT COALESCE(rp.is_bot, false)
    ) >= 2;
  IF FOUND THEN
    RETURN;
  END IF;
  ...
  IF humans >= 2 THEN
    UPDATE public.rooms SET status = 'playing' WHERE id = p_room_id AND status = 'waiting';
    RETURN;
  END IF;
...
BEGIN
  ...
  WHERE u.id = p_room_id
    AND u.status = 'waiting'
    AND COALESCE(u.matchmaking_pool, true) = true
    AND (
      SELECT COUNT(*)::int
      FROM public.room_players rp
      WHERE rp.room_id = u.id AND NOT COALESCE(rp.is_bot, false)
...
BEGIN
  ...
  IF rid IS NOT NULL THEN
    ...
    PERFORM public.durak_force_start_if_two_humans(rid);
    RETURN NEXT;
    RETURN;
  END IF;
  ...
  IF FOUND THEN
    ...
    PERFORM public.durak_force_start_if_two_humans(room_rec.id);
