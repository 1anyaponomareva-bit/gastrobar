-- Гонка двух одновременных «быстрый поиск»: оба не видят чужую новую комнату и создают две пустые —
-- один потом получает бота, второй висит один. Сериализуем критическую секцию поиска/создания комнаты.
