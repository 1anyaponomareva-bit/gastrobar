-- PostgREST/Supabase: надёжный вызов RPC — одна функция с одним аргументом jsonb
-- (избегает путаницы с порядком имён параметров и кэшем схемы).
-- Выполни этот файл в Supabase SQL Editor, если старая durak_join_queue(text, text) ещё есть.
...
-- После применения: Dashboard → Settings → API → «Restart» или подождать ~1 мин (кэш PostgREST).
