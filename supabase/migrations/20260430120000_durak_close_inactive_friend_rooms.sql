-- Столы с друзьями (matchmaking_pool = false): если нет активности 20 минут — status = finished.
-- Активность = max(created_at комнаты, updated_at room_state, last_seen_at / joined_at участников).
-- Быстрая очередь не затрагивается.
--
-- Дополнительно: при каждом durak_player_ping в friend-комнате запускается эта уборка
-- (без pg_cron тоже со временем закроет «забытые» столы).
-- Опционально в Supabase (pg_cron): select cron.schedule('durak-friend-idle', '*/10 * * * *', $$select public.durak_close_inactive_friend_rooms()$$);
...
-- Пинг в лобби нет — закрытие «протухших» столов при действиях в меню друзей.
