...
const GASTROBAR_DAY_ITEMS: ShiftChecklistItem[] = [
  // Block 1 — Opening 8–10
  ...[
    ["stock-1", "Проверены остатки продукции"],
    ["stock-2", "Проверены сроки годности"],
  ...
  ...[
    ...
    barDay(`gb-day-${id}`, "Открытие смены 8–10 утра", "Уборка снаружи", label),
  ),
  ...[
    ...
    barDay(`gb-day-${id}`, "Открытие смены 8–10 утра", "Уборка внутри", label),
  ),

  // Block 2 — During shift 10–17
  ...[
    ...
    barDay(
      `gb-day-${id}`,
      "Контроль в течение смены с 10 до 17",
      "",
      label,
    ),
  ),

  // Block 3 — Closing 17–18
  ...[
    ["close-stock-1", "Подсчитаны остатки продукции"],
    ["close-stock-2", "Передана информация сменщику об остатках"],
  ].map(([id, label]) =>
    barDay(`gb-day-${id}`, "Закрытие смены с 17 до 18", "Остатки и заказ", label),
  ),
  ...[
    ...
    barDay(`gb-day-${id}`, "Закрытие смены с 17 до 18", "Уборка внутри", label),
  ),
  ...[
    ["close-out-1", "Протерты наружные поверхности"],
    ["close-out-2", "Убрана территория вокруг трака"],
  ].map(([id, label]) =>
    barDay(`gb-day-${id}`, "Закрытие смены с 17 до 18", "Уборка снаружи", label),
  ),
  ...[
    ["close-trash-1", "Весь мусор вынесен"],
    ["close-trash-2", "Установлен новый мусорный пакет"],
  ].map(([id, label]) =>
    barDay(`gb-day-${id}`, "Закрытие смены с 17 до 18", "Мусор", label),
  ),
  ...[
    ["close-end-1", "Касса посчитана"],
    ["close-end-2", "Отчет о закрытии смены и чеки отправлены"],
  ].map(([id, label]) =>
    barDay(`gb-day-${id}`, "Закрытие смены с 17 до 18", "Закрытие", label),
  ),
];
