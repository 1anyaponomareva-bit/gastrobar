...
const FLAG_SRC: Record<"ru" | "vn", string> = {
  ru: "/flags/ru.svg",
  vn: "/flags/vn.svg",
};

const LANGS: Array<"ru" | "vn"> = ["ru", "vn"];

function langOptionKey(id: AppLang): `lang_option_${AppLang}` {
  return `lang_option_${id}`;
}

export function StaffTestLanguageFlags() {
  const { lang, changeLang, t } = useTranslation();
  const staffTestLang = toStaffTestAppLang(lang);
